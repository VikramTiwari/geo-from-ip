...
// get node packages
const debug = require('debug')('geo-from-ip')
const path = require('path')
const mmdbreader = require('maxmind-db-reader')
const city = mmdbreader.openSync(path.resolve(path.join(__dirname, '/../mmdb/GeoLite2-City.mmdb')))
...
function allData (ip) {
  ...
  debug(`returning location result: ${result}`)
  // return result
  return result
}
